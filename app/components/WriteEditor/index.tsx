...
const WriteEditor: FC<WriteEditorProps> = ({ categoryList }): JSX.Element => {
  ...
  const handleOnSubmit = async () => {
    try {
      if (!value || value.trim() === "")
        return toast.error(`post description not provided`);
      if (!postCat || postCat.trim() === "")
        return toast.error(`post category not provided`);
      ...
      <div className={styles.wrapper}>
        ...
        <div className={styles.categoryWrapper}>
          <select
            ...
            value={postCat}
            onChange={(e: any) => setPostCat(e.target.value)}
          >
            <option value={""}>Select Post Category</option>
            {categoryList?.map((self, index) => (
              <option className={styles.options} value={self.slug} key={index}>
                {self.title}
              </option>
            ))}
          </select>
